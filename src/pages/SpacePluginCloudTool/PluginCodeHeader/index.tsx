      ...
      >
        ...
        <div className={cx(styles['bottom-box'], 'flex', 'items-center')}>
          <span className={cx(styles.box)}>
            {pluginInfo?.type === PluginTypeEnum.HTTP ? 'http' : dict('PC.Pages.SpacePluginCloudTool.code')}
          </span>
          <span className={cx(styles.box)}>
            {pluginInfo?.publishStatus === PublishStatusEnum.Published
          ...
          <span className={cx(styles['update-time'])}>
            {dict('PC.Pages.SpacePluginCloudTool.savedAt')}{dayjs(pluginInfo?.modified).format('HH:mm')}
          </span>
        </div>
      </section>
      <Segmented
